...
class TestEndToEndWorkflow:
    ...
    def test_error_handling_workflow(self, db_session, report_service):
        ...
        # 1. Create template with invalid structure - استخدام كائن غير قابل للتسلسل
        print("\n🔴 Testing invalid structure...")
        
        # Test: structure is not a list (string)
        result = report_service.create_template(
            title="قالب خاطئ",
            structure="invalid_string",  # string بدلاً من list
            description="هذا القالب سيفشل"
        )
        # هذا قد ينجح لأن json.dumps يقبل string
        # نختبر حالة فشل حقيقية
        ...
        result = report_service.create_template(
            title="قالب خاطئ",
            structure=[NonSerializable()],  # كائن غير قابل للتسلسل
            description="هذا القالب سيفشل"
        )
        # هذا يجب أن يفشل
