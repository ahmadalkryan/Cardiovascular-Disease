...
class TestDataService:
    ...
    def test_save_patient_data_with_nan(self, data_service):
        """اختبار حفظ بيانات تحتوي على NaN"""
        patient_data = {
            'age': float('nan'),
            'sex': 1,
            'resting_bp': 140,
    ...
    def test_get_patients_data_by_model(self, data_service):
        """اختبار جلب بيانات المرضى حسب النموذج"""
        # حفظ بيانات لنموذجين مختلفين
        data_service.save_patient_data(
            patient_data={'age': 45},
            prediction=1,
            probability=0.90,
    ...
    def test_clean_data_for_json(self, data_service):
        """اختبار تنظيف البيانات للتحويل إلى JSON"""
        # إنشاء DataFrame مع بيانات متنوعة
        df = pd.DataFrame([
    ...
    def test_get_statistics_with_data(self, data_service):
        """اختبار الإحصائيات عند وجود بيانات"""
        # حفظ بيانات متعددة
        for i in range(5):
            data_service.save_patient_data(
